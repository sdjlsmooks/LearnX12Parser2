from x12.parser import X12Parser
...
def extract_sac_loops(parsed_data):
    ...
    for segment in parsed_data:
        if isinstance(segment, list):  # Check nested data structures
            sac_loops.extend(extract_sac_loops(segment))
        elif isinstance(segment, dict) and segment.get('segment', '').startswith('SAC'):
